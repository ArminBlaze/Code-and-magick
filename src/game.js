...
Game.prototype = {
  ...
  _drawPauseScreen: function() {
    switch (this.state.currentStatus) {
      case Verdict.WIN:
        this.setCanvas('Да, детка! Попал в цель как настоящий снайпер!');
        break;
      case Verdict.FAIL:
        this.setCanvas('Не расстраивайся, в следующий раз получится!');
        break;
      case Verdict.PAUSE:
        this.setCanvas('Сделай паузу, скушай твикс!');
        break;
      case Verdict.INTRO:
        this.setCanvas('Пендальф - молодец! П е н д а л ь ф - моё имя!');
        break;
    }
  },

  createArrayLines: function(text, messageWidth) {
    var linesArray = [];
    ...
    words.forEach(function(word) {
      ...
      var testLineWidth = this.ctx.measureText(testLine).width;
  ...
  drawCanvas: function(cordX, cordY, numberOfLines, fillStyle) {
    this.ctx.beginPath();
    this.messageHeight = numberOfLines * this.getFontHeight(fontName);
    this.ctx.moveTo(cordX, cordY); //лево, низ
    this.ctx.lineTo(cordX + 255, cordY - 20); //право, низ
    this.ctx.lineTo(cordX + 255, cordY - 40 - this.messageHeight);//право, верх
    this.ctx.lineTo(cordX + 25, cordY - 40 - this.messageHeight);//лево, верх
    this.ctx.closePath();
    this.ctx.fillStyle = fillStyle;
    this.ctx.fill();
  },
  drawText: function() {
    //определяем стиль и параметры выводимого текста
    this.ctx.fillStyle = 'rgba(0, 0, 0, 1)';
    this.ctx.font = fontName;
    var marginLeft = this.x + 30;
    var marginTop = this.y - 20 - this.messageHeight;

    var lineHeight = this.getFontHeight(fontName);
  ...
  setCanvas: function(text) {
     ...
     * @type {number}
     */
    this.messageWidth = 85;
    /** координаты левого нижнего угла сообщения */
    this.x = 310;
    this.y = 250;
    /** ширина тени */
    this.shadowShift = 10;

    /** формируем массив строк ссобщения      */
    this.ArrayLines = this.createArrayLines(text, this.messageWidth);

    /** Рисуем теневой контур */
    this.shadowColor = 'rgba(0, 0, 0, 0.7)';
    this.drawCanvas(this.x + this.shadowShift, this.y + this.shadowShift, this.ArrayLines.length, this.shadowColor);

    /** Рисуем основной контур */
    this.mainColor = '#FFFFFF';
    this.drawCanvas(this.x, this.y, this.ArrayLines.length, this.mainColor);
...
/** @param  {boolean} runShift */
var shiftElement = function(runShift) {
  if (runShift) {
    headerClouds.style.backgroundPosition = headerClouds.getBoundingClientRect().top + 'px';
  }
};

window.addEventListener('scroll', function() {
  if (Date.now() - lastCall >= THROTTLE_DELAY) {
    var isCloudsVisible = utils.isElementVisible(headerClouds);
    lastCall = Date.now();
  }
  shiftElement(isCloudsVisible);
